...
export default async function ClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const org = await db.organization.findUnique({
    where: { id },
    include: {
      users: { where: { clerkId: { not: { startsWith: "pending_" } } }, select: { id: true, firstName: true, lastName: true, email: true, role: true, createdAt: true, _count: { select: { participations: true } } } },
      ttxSessions: { take: 10, orderBy: { createdAt: "desc" }, select: { id: true, title: true, status: true, theme: true, createdAt: true, _count: { select: { participants: true } } } },
      profile: true,
  ...
  const hasUsers = org.users.length > 0;
  const hasExercise = org.ttxSessions.length > 0;
  const onboarding = [
    { label: "Company profile", done: hasProfile },
    { label: "Security stack", done: hasTools },
    { label: "Users invited", done: hasUsers || pendingInvites.length > 0 },
    { label: "First exercise", done: hasExercise },
  ];
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-1">{org.slug}.threatcast.io · {org.plan}{org.isDemo ? " · Demo" : ""}{org.maxTtxPerMonth >= 9999 ? " · Unlimited" : ""}</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-6">
        ...
        <div className="cyber-card text-center"><p className="font-display text-2xl font-bold text-cyan-400">{pendingInvites.length}</p><p className="text-gray-500 text-xs">Pending</p></div>
        ...
        <div className="cyber-card text-center"><p className="font-display text-2xl font-bold text-yellow-400">{org.ttxUsedThisMonth}/{org.maxTtxPerMonth >= 9999 ? "∞" : org.maxTtxPerMonth}</p><p className="text-gray-500 text-xs">Used/Limit</p></div>
      </div>

      {/* Invite users to this portal */}
      <AdminInviteForm orgId={org.id} orgName={org.name} />

      {/* Pending invitations */}
      {pendingInvites.length > 0 && (
        <div className="cyber-card mb-4 border-yellow-500/20">
          <h3 className="text-yellow-400 text-xs font-semibold mb-2">Pending Invitations ({pendingInvites.length})</h3>
          <div className="space-y-1">{pendingInvites.map((p, i) => (
            <div key={i} className="flex items-center justify-between py-1">
              <span className="text-gray-400 text-xs">{p.email}</span>
              <span className="text-gray-600 text-xs">{new Date(p.createdAt).toLocaleDateString("en-GB")}</span>
            </div>
          ))}</div>
        </div>
      )}
      ...
      <div className="cyber-card mb-4">
        <div className="flex items-center justify-between mb-2"><h3 className="text-white text-sm font-semibold">Onboarding</h3><span className="text-cyber-400 text-sm font-mono">{stepsComplete}/4</span></div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="cyber-card">
          ...
          {org.users.length === 0 ? <p className="text-gray-500 text-xs">No users yet — send invitations above</p> :
          <div className="space-y-2">{org.users.map(u => (
            <div key={u.id} className="flex items-center justify-between py-1.5 border-b border-surface-3/50 last:border-0">
              ...
              <div className="flex items-center gap-2"><span className="text-gray-500 text-xs">{u._count.participations} ex</span><span className={`cyber-badge text-xs ${rc[u.role]}`}>{rl[u.role]}</span></div>
            </div>
          ))}</div>}
        </div>
        ...
        {/* Sessions */}
        <div className="cyber-card">
          <h2 className="text-white text-sm font-semibold mb-3">Exercises ({org.ttxSessions.length})</h2>
          {org.ttxSessions.length === 0 ? <p className="text-gray-500 text-xs">No exercises yet</p> :
          <div className="space-y-2">{org.ttxSessions.map(s => (
