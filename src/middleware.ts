...
function setSecurityHeaders(res: NextResponse) {
  ...
  res.headers.set("Content-Security-Policy", "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https: blob:; font-src 'self' https://fonts.gstatic.com; connect-src 'self' https://api.anthropic.com https://api.openai.com https://generativelanguage.googleapis.com wss://*.pusher.com https://api.resend.com; frame-src 'self'; frame-ancestors 'none'");
}
...
export async function middleware(req: NextRequest) {
  ...
  if (subdomain) {
    ...
    if (!pathname.startsWith("/portal") && !pathname.startsWith("/api") && !pathname.startsWith("/sign")) {
      ...
      const rwRes = NextResponse.rewrite(url, { headers });
      setSecurityHeaders(rwRes);
      return rwRes;
    }
    const subRes = NextResponse.next({ headers });
    setSecurityHeaders(subRes);
    return subRes;
  }
